...
## Part 1 - Cloudinary Account, Backend, and Dependencies
...
- Create the neccessary files and code, *view the repo files for guidance and comments*
    ...
    - In the `services` folder, create a file called `cloudinary.js`, this file will require the cloudinary dependency, configure it and define a function which we will use later.
    - In the `routes` folder, create a file called `image.js`, this will be a regular router structure along with some functionality of a controller for the POST Method
    - In the root of your Backend app, create a `.env` file (if you don't already have one)
        - Since a `.env` file is not uploaded to this repo, I have provided the basic structure required. You will need to replace the values with your own sensitive information. Your cloudinary information is available to you on the Cloudinary Dashboard page.
            ```
            # MongoDB
            PORT=ANYPORTNUMBER
...
- Import the new component into your parent component, in my case imported it to `App.jsx`.
- Import Axios into your `UploadImage.js` component
- Define some states, a `handleSelectFile()` function, a `handleUpload()` function, and start coding.
    - Read about the multer dependency, it uses req.file, req.files, req.file.length, etc.
- In the `return` section of your component, make sure your `input` tag matches the input tag in my repo.
    - `Multiple={true}` allows you to select multiple files, `false` means you can only select 1 file
- In the `return` section, make sure your `button` tag calls the `handleUpload()` function in the `onClick`
